...
//        FileInputStream secKeyIn = new FileInputStream("/Users/biafra/andlocator/tools/keymanager/test-data/secret.asc");
    ...
    public long getKeyId() {
        ...
        return GetKeyId(new File("/sdcard/pub.asc"));
    }

    public static long GetKeyId(final File file) {
        FileInputStream keyIn = null;
        try {

            keyIn = new FileInputStream(file);

        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
        return GetKeyId(keyIn);
    }

    public static long GetKeyId(final InputStream is) {
        try {
            PGPPublicKey pubKey = readPublicKey(is);
    ...
    public static long GetKeyId(final String key) {

        try {
            PGPPublicKey pubKey = readPublicKey(new ByteArrayInputStream(key.getBytes()));
            return pubKey.getKeyID();
    ...
    public static PGPPublicKey GetKey(File file) {

        PGPPublicKey result = null;
        try {

            result = readPublicKey(new FileInputStream(file));
